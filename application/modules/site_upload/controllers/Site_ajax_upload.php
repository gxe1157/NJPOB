...
function __construct() {
    ...
    $table_list = ['users_upload', 'car_shields_upload'];
    $table_opt  = $this->input->post('table_opt', TRUE);
    $this->table_name = $table_list[ $table_opt ];
    $this->controller = $this->table_name;
    $this->source_id  = $this->input->post('manage_rowid', TRUE) != null ? $this->input->post('manage_rowid', TRUE) : 0;   
ddf('source_id | '.$this->source_id);
 // quit($this->table_name.' | '.$this->source_id)  ;
...
function ajax_remove_one()
{
    ...
    /* full upload path */
    $upload_path = $this->_build_upload_folder();
    $query = $this->get_where($img_id, $this->table_name);
    ...
    if($data['success'] == 1)
       $this->model_name->_delete($img_id, $this->table_name);      
...
function ajax_upload_one()
{
    ...
    if( $is_uploaded == false ){
        ...
        if( $this->upload->do_upload('file') ) {
          ...
          $data['success'] = 1;
          $data['error_mess'] = '';
          $data['update_rec'] = $this->_update_img_data($user_id, $table_data);
        } else {
          // display errors
...
function _update_img_data($user_id, $table_data)
{
    $table_name = $this->table_name;
    $table_data['source_id'] = $this->source_id;

    /* Update database */
    $update_rec = $this->model_name->update_data($user_id, $table_data, $table_name);
...
{
    $prd_folder  = $this->controller ? $this->controller."/" : null;
